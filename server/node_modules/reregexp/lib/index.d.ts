...
export declare type $N = `$${1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9}`;
export declare type Flag = 'i' | 'm' | 'g' | 'u' | 'y' | 's' | 'd';
...
export declare type Result = Pick<ReRegExp, 'rule' | 'lastRule' | 'context' | 'flags'> & {
    queues: RegexpPart[];
};
export declare type UPCData = {
    negate: boolean;
    short: boolean;
    key?: string;
    value: string;
};
export declare type UPCFactory = (data: UPCData) => UPCInstance | never;
...
export declare type CharsetCacheType = CharsetNegatedType | typeof SYMBOL_DOTALL | typeof SYMBOL_ALL;
export declare type CharsetCache = {
    [key in CharsetCacheType]?: CodePointResult;
};
export declare type CodePointRangeItem = [number, number] | [number];
export declare type CodePointRanges = Array<CodePointRangeItem>;
