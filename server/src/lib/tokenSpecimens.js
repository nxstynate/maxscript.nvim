...
exports.emmitTokenValue = void 0;
var ReRegExp = require('reregexp')["default"];
/*
export{}
declare global {
    interface Array<T> {
...
var rx = function (rule) {
    var lit = new ReRegExp(new RegExp(rule));
    return lit.build();
};
var emmitTokenValue = function (r) { return ({
    comment_SL: rx("[-]{2}[A-Za-z0-9_]{".concat(r >= 3 ? r - 2 : 0, "}")),
    comment_BLK: rx("\\/\\*[a-z]{".concat(r >= 5 ? r - 4 : 0, "}\\*\\/")),
    string: rx("\"(?:[A-Za-z0-9_]{".concat(r >= 3 ? r - 2 : 0, "})\"")),
    ws: rx("[ \t]{".concat(r, "}")),
    newline: rx("[;\r\n]{".concat(r, "}")),
    locale: rx("~[A-Za-z0-9_]{".concat(r >= 2 ? r - 2 : 0, "}~")),
    path: rx("\\$[A-Za-z0-9_]{".concat(r > 1 ? r - 1 : 0, "}")),
    name: rx("#[A-Za-z0-9_]{".concat(r > 1 ? r - 1 : 0, "}")),
    identity: rx("[A-Za-z_][A-Za-z0-9_]{".concat(r > 1 ? r - 1 : 0, "}")),
    sharp: '#',
    questionmark: '?',
    lparen: '(',
    rparen: ')',
    ...
    lbrace: '{',
    rbrace: '}',
    comparison: rx("(?:(=[=!><])|[><]){".concat(r, "}")),
    assign: rx("=[-+/*]{".concat(r > 1 ? r - 1 : 0, "}")),
    math: rx("[-+/*]{".concat(r, "}")),
    time: rx("[0-9][:][0-9][.][0-9]{".concat(r >= 4 ? r - 4 : 0, "}")),
    number: rx("[0-9]{".concat(r, "}")),
    bitrange: '..',
    delimiter: '.',
    sep: ',',
    param: ':',
...
exports.emmitTokenValue = emmitTokenValue;
